...
export async function probeGatewayLive(config) {
  const port = config?.gateway?.port || 18789;
  const results = [];

  // ── PROBE 1: Is gateway actually running? ─────────────────────────────────
  const running = await tcpProbe('127.0.0.1', port);
  results.push({
  ...
  const wsResult = await wsProbeNoAuth('127.0.0.1', port);
  ...
  const corsRes = await httpOptions(`http://127.0.0.1:${port}/`, evilOrigin);
  if (corsRes) {
